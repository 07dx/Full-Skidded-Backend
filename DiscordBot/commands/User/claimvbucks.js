...
module.exports = {
    commandInfo: {
        name: "claimvbucks",
        description: "Pegue os seus 200 V-bucks"
    },
        ...
        try {
            ...
            if (lastClaimed && (Date.now() - new Date(lastClaimed).getTime() < 24 * 60 * 60 * 1000)) {
                ...
                return interaction.followUp({
                    content: `Você ja pegou seus **V-Bucks.** Tente novamente em: **${timeLeft} hours.**`,
                    ephemeral: true
                });
            }

            const filter = { accountId: user?.accountId };
            const updateCommonCore = { $inc: { 'profiles.common_core.items.Currency:MtxPurchased.quantity': 200 } }; //200 is vbucks for day but u can change it
            const updateProfile0 = { $inc: { 'profiles.profile0.items.Currency:MtxPurchased.quantity': 200 } }; //200 is vbucks for day but u can change it
            ...
            const embed = new MessageEmbed()
                .setTitle("Daily V-Bucks Adquiridos!")
                .setDescription(`Você pegou seus **200 V-Bucks**!`)
