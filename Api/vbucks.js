...
app.get("/api/skibidireload/vbucks", async (req, res) => {
    const { username, reason, apiKey } = req.query;

    if (!username) {
        return res.status(400).json({ code: "400", error: "Hell no." });
    }
    if (!reason) {
        return res.status(400).json({ code: "400", error: "Hell no." });
    }
    if (!apiKey || apiKey != "Key-7656237846Tzy87655DoNotChangeDueToArena") {
        return res.status(400).json({ code: "400", error: "Hell no." }); // I am just making the most basic changes so ppl cant exploit this
    }
