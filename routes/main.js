...
app.get("/api/v1/events/Fortnite/download/:accountId", async (req, res) => { // If anybody touches this code, ill touch him back -tzy
    const accountId = req.params.accountId;
    try { 

        const use111 = await User.findOne({ accountId });
        if (!use111) {
            console.log("[ANTI-EXPLOIT] Exploiter gay:", req.socket.remoteAddress)
            return;
        }
        ...
        if (!playerData) {
            playerData = await Arena.findOneAndUpdate(
                { accountId: accountId },
                { $set: { hype: 0 } },
        ...
        const playerHype = playerData.hype || 0;

        const filePath = path.resolve(__dirname, '../responses/s12arena.json'); // Idk, works for s11 - s13
        let fileContents;
        try {
            fileContents = fs.readFileSync(filePath, 'utf-8');
        } catch (fileErr) {
            console.log("Error reading the JSON file:", fileErr.message);
            return res.status(500).send("Internal Server Error: Unable to read JSON file.");
        }

        let arenaData;
        try {
            arenaData = JSON.parse(fileContents);
        } catch (parseErr) {
            console.log("Error parsing JSON file:", parseErr.message);
            return res.status(500).send("Internal Server Error: JSON parsing failed.");
        }

        const currentSeason = "S12";

        let playerDivision = [];
        if (playerHype >= 12200) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division10`];
        } else if (playerHype >= 10000) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division9`];
        } else if (playerHype >= 8900) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division8`];
        } else if (playerHype >= 7400) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division7`];
        } else if (playerHype >= 5700) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division6`];
        } else if (playerHype >= 4000) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division5`];
        } else if (playerHype >= 2750) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division4`];
        } else if (playerHype >= 1600) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division3`];
        } else if (playerHype >= 600) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division2`];
        } else if (playerHype < 600) {
            playerDivision = [`LG_ARENA_${currentSeason}_Division1`];
        }

        const replaceInJson = (obj) => {
            if (typeof obj === "string") {
                return obj.replace(/tzyseason/g, currentSeason); 
            }
            ...
            if (obj && typeof obj === "object") {
                for (let key in obj) {
                    obj[key] = replaceInJson(obj[key]);
                }
            }
            return obj;
        };

        arenaData = replaceInJson(arenaData);
        arenaData.player.persistentScores.Hype = playerHype;
        arenaData.player.accountId = accountId;
        arenaData.player.tokens = playerDivision;
        res.json(arenaData);
        console.log("[ARENA] Username:", use111.username, "Hype:", playerHype);
    } catch (error) {
...
app.get("/api/tzy/elimination/:apikey/:name", async (req, res) => { // If anybody fucking touches this code, i will touch back but hard
    try {
        if (req.params.apikey === "" || req.params.apikey === null || !req.params.apikey)
            return;
        if (req.params.apikey != "Key-7656237846Tzy87655DoNotChangeDueToArena")
            return;
        if (!req.params.name || req.params.name === null)
            return;
        const user = await User.findOne({ username: req.params.name }).lean();
        if (!user) {
            return res.status(400).send("invalid");
        }
        const playerData = await Arena.findOne({ accountId: user.accountId }).lean();
        if (!playerData) {
            return console.log("arena profile not found");
        }
        let hypeIncrement = 50;
        let vbucksIncrement = 50;
        await Arena.findOneAndUpdate({ accountId: user.accountId }, { $inc: { hype: hypeIncrement } }, { upsert: true, new: true, setDefaultsOnInsert: true });
        await Profiles.findOneAndUpdate(
            { accountId: user.accountId },
            { $inc: { 'profiles.common_core.items.Currency:MtxPurchased.quantity': vbucksIncrement } }
        );
        await axios.post("http://127.0.0.1:3013/fortnite/api/game/v3/profile/*/client/emptygift", {
            ...
            personalMessage: "Love from Tzy",
            accountId: user.accountId,
            playerName: req.params.name
        });
        console.log("[API] Added Arena Points and Vbucks For Elimination To:", req.params.name);
